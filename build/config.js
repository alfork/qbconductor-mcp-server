...
for (let i = 0; i < args.length; i++) {
    if (args[i] === '--env' && i + 1 < args.length) {
        ...
        if (key === 'CONDUCTOR_SECRET_KEY')
            envArgs.conductorSecretKey = value;
        if (key === 'CONDUCTOR_API_KEY')
            envArgs.conductorApiKey = value;
        if (key === 'CONDUCTOR_END_USER_ID')
            envArgs.conductorEndUserId = value;
        if (key === 'CONDUCTOR_API_BASE_URL')
            envArgs.conductorApiBaseUrl = value;
        if (key === 'LOG_LEVEL')
            envArgs.logLevel = value;
        if (key === 'CACHE_TTL_MINUTES')
            envArgs.cacheTtlMinutes = value;
        if (key === 'CACHE_MAX_SIZE')
            envArgs.cacheMaxSize = value;
