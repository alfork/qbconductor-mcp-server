...
export const getAccountTaxLinesTool: Tool = {
...
export const getVendorSpendingAnalysisTool: Tool = {
...
export const getAccountTaxLinesHandler = withErrorHandling(async (input: unknown) => {
...
export const generateFinancialSummaryHandler = withErrorHandling(async (input: unknown) => {
  ...
  const summary = {
    ...
    balancesByType: {} as Record<string, number>,
    totalBalance: 0,
    activeAccounts: 0,
    inactiveAccounts: 0,
    dateRange: {
...
export const getVendorSpendingAnalysisHandler = withErrorHandling(async (input: unknown) => {
  const params = validateInput(GetVendorSpendingAnalysisSchema, input);
  
  const client = new ConductorClient(params.endUserId);
  
  const billQueryParams: Record<string, any> = {};
  const paymentQueryParams: Record<string, any> = {};
  ...
  const vendorAnalysis: Record<string, {
    vendorName: string;
    totalBilled: number;
    totalPaid: number;
    billCount: number;
  ...
  [...checkPayments, ...creditCardPayments].forEach((payment: any) => {
  ...
  const summary = formatListResponse(
    analysisResults,
    (vendor) => formatVendorSpending(vendor.vendorName, vendor.totalBilled, vendor.billCount, vendor.paymentCount),
